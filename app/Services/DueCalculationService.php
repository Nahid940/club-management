...
class DueCalculationService
{
    ...
    {
        $this->fees_schedule=New MembershipFeeSchedule();
    }
    ...
    public function getDueOfMember($member_id)
    {
        ...
        {
            $reg_date=date('Y-m-01',strtotime($member_registration_date->registration_date));
            $date1=date('Y-m-01',strtotime('+1 month',strtotime($reg_date)));
        }else
        {
        ...
        {
            $current_date=date('Y-m-01',strtotime($current_date));
        ...
        $fees=$this->fees_schedule->fees($admission_fee->member_type,intval($current_month));
        $total_payable=0;
...
//        die;
    ...
    public function memberDueCalculation($admission_fee,$member_id,$membership_type_id)
    {
        ...
        $fees=$this->fees_schedule->fees($membership_type_id,intval($current_month));
        $total_payable=0;
        for ($year = $year1; $year <= $year2; $year++) {
            $start_month = ($year == $year1) ? $month1 : 1;
            $end_month = ($year == $year2) ? $month2 : 12;
...
//                }
        ...
        $monthly_fee_due=($total_payable-$total_payment)<0?0:$total_payable-$total_payment;
        return array(
...
//
            ...
            $fees=$fees_schedule->fees($member->member_type,intval($to_month));
            $fee_amount=0;
    ...
    public function getMembersPaymentSchedule($data,$fees_schedule)
    {
        $where=[];
